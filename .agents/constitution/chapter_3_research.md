...
*   **Tier 3 (Industry Intermediaries):** Distributor catalogs, trade association directories, and B2B wholesale platforms.
...
Every piece of evidence is mathematically scored to communicate its reliability instantly:
*   **100%:** Backed by a Tier 1 source or mathematical certainty.
*   **95%:** Backed by an official Tier 2 source, corroborated by secondary evidence.
*   **85%:** Backed by multiple Tier 3 sources.
*   **60%:** Single Tier 3 source, or an uncorroborated Tier 2 claim.
*   **25%:** AI inference or an unverified marketing claim.
*   **0%:** Explicitly unknown or null.

### 3.5 Conflict Resolution
1. **Conflicts are Assets:** Contradictory data is not a bug; it is actionable intelligence. If a brand claims 98% purity but their customs records suggest a supplier providing 85% purity, the system has uncovered a vulnerability.
2. **No Averaging or Guessing:** When two sources conflict, the system must never attempt to average the numbers or guess the "truth."
3. **Explicit Flagging:** Both conflicting records are preserved, and the parent entity is immediately flagged with a `CONFLICT DETECTED` status.

### 3.6 Manual Review Workflow
1. **Automated Escalation:** Any critical field marked as `CONFLICT DETECTED` or any expiring Tier 1 document must be automatically routed to the Operations team.
2. **Contextual Task Generation:** The system must generate a review task that explicitly links the conflicting sources side-by-side, preventing the reviewer from having to hunt for context.
3. **Human Resolution:** A human operator must physically verify the documentation. When resolving the conflict, the operator must append a permanent note explaining the resolution logic, which becomes part of the entity's immutable audit trail.
